...
document.querySelector('.access-link').addEventListener('click', () => {
    ...
    const win = frame.contentWindow;
    if (win.__uv) {
        navigator.clipboard.writeText(new URL('./?link=' + encodeURIComponent(btoa(win.__uv.location.href)), location.href).href);
    };
});
...
document.querySelector('.access-panel').addEventListener('mouseenter', async event => {
    const frame = document.querySelector('.access-frame');
    const win = frame.contentWindow;
    if (window.location.hash !== '#gs' && win && win.__uv) {
        ...
        const favi = document.querySelector.call(win.document, '.access-panel img');
        if (favi && Object.getOwnPropertyDescriptor(HTMLLinkElement.prototype, 'href').get.call(favi)) {
            const res = await win.__uv.client.fetch.fetch.call(win, Object.getOwnPropertyDescriptor(HTMLLinkElement.prototype, 'href').get.call(favi));
            const blob = await res.blob();
            const url = URL.createObjectURL(blob);
            document.querySelector('.access-panel .controls .icon').src = url;
            URL.revokeObjectURL(url);
        } else {
            const res = await win.__uv.client.fetch.fetch.call(win, win.__uv.rewriteUrl('/favicon.ico'));
            const blob = await res.blob();
            const url = URL.createObjectURL(blob);
            document.querySelector('.access-panel .controls .icon').src = url;
            URL.revokeObjectURL(url);
...
window.nav = function nav(url, title = '', ico) {
    app.main.target.style.display = 'none';
    app.header.target.style.display = 'none';
    const frame = document.querySelector('.access-frame');
    document.querySelector('.access-panel .controls input').value = title;
    if (ico) document.querySelector('.access-panel img').src = ico;
    frame.src = url;
    frame.style.display = 'block';
    document.querySelector('.access-panel').style.removeProperty('display');
};
